...
func TestEmptySummary(t *testing.T) {
	...
	var summary = runner.NewSummary()

	assertion.NewName("get correct name").
		WithExpected(runner.DEFAULT_SUMMARY_NAME).
...
func TestNamedSummary(t *testing.T) {
	...
	var summary = runner.NewNamedSummary("test")
...
func TestSingleInfoSummary(t *testing.T) {
	var assertion = xtests.New(t)
	var summary = runner.NewSummary().
		Add(runner.NewInformation("info#1").
			SetStatus(runner.INVALID).
...
func TestMultipleInfoSummary(t *testing.T) {
	...
	var summary = runner.NewSummary().
		Add(runner.NewInformation("info#1").
			SetStatus(runner.SUCCESS).
			SetDuration(5 * time.Second).SetError(errors.New("test"))).
		Add(runner.NewInformation("info#2").
			SetStatus(runner.DISABLED).
			SetDuration(1 * time.Millisecond)).
		Add(runner.NewInformation("info#3").
			SetStatus(runner.SUCCESS).
			SetDuration(791 * time.Microsecond)).
		Add(runner.NewInformation("info#4").
...
func TestGroupInfoSummary(t *testing.T) {
	var assertion = xtests.New(t)
	var summary = runner.NewSummary().
		Add(runner.NewInformation("info#01").
			SetStatus(runner.INVALID).
			SetDuration(5*time.Second).SetError(errors.New("test"))).
		Add(runner.NewInformation("info#02").
			SetStatus(runner.DISABLED).
			SetDuration(1*time.Millisecond)).
		AddGroup("test#1",
			runner.NewInformation("info#11").
				SetStatus(runner.DISABLED).
				SetDuration(412*time.Millisecond),
			runner.NewInformation("info#12").
				SetStatus(runner.INVALID).
				SetDuration(13*time.Nanosecond).
				SetError(errors.New("test")),
		).
		AddGroup("test#2",
			runner.NewInformation("info#21").
				SetStatus(runner.SUCCESS).
				SetDuration(213*time.Microsecond),
			runner.NewInformation("info#22").
				SetStatus(runner.SUCCESS).
